...
public class DogePriceClient
{
    private readonly HttpClient _http;
    private readonly ILogger<DogePriceClient> _logger;

    public DogePriceClient(HttpClient http, ILogger<DogePriceClient> logger)
    {
        _http = http;
        _logger = logger;
    }

    public async Task<DogePriceStats?> FetchAsync(CancellationToken ct = default)
    {
        try
        {
        ...
        {
            _logger.LogError(ex, "Failed to fetch DOGE price from CoinPaprika");
            return null;
        }
    }
