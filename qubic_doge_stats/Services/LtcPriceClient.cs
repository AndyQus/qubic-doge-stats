...
public class LtcPriceClient
{
    private readonly HttpClient _http;
    private readonly ILogger<LtcPriceClient> _logger;

    public LtcPriceClient(HttpClient http, ILogger<LtcPriceClient> logger)
    {
        _http = http;
        _logger = logger;
    }

    public async Task<LtcPriceStats?> FetchAsync(CancellationToken ct = default)
    {
        try
        {
        ...
        {
            _logger.LogError(ex, "Failed to fetch LTC price from CoinPaprika");
            return null;
        }
    }
